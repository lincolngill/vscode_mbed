...
def get_args():
    ...
    parser.add_argument("mbed_project_name", 
        help="mbed project name. Creates a new project directory <workspace_dir>/<mbed_project_name> if does not exist")
    ...
    parser.add_argument("-w", "--workspace_dir", 
        help="Directory location of the vscode workspace", 
        default=os.path.join(os.path.expanduser("~"),"mbed"))
...
class Project:
    ...
    def create_project(self):
        ...
        if not os.path.exists(self.projectdir):
            old_dir = os.getcwd()
            print "chdir %s" % self.workspace.workspace_dir
            os.chdir(self.workspace.workspace_dir)
            os_cmd("mbed new %s" % self.name)
            os.chdir(self.projectdir)
            os_cmd("mbed toolchain %s" % self.toolchain)
            os_cmd("mbed target %s" % self.target)
            os_cmd("mbed config --list")
            os_cmd("mbed export -m %s -i %s --source \".\" --source \"../mbed-os\"" % (self.target, self.ide))
    ...
    def __init__(self, args):
        ...
        """
        self.name = args.mbed_project_name
        self.workspace = Workspace(args.workspace_dir)
        self.projectdir = os.path.join(self.workspace.workspace_dir, self.name)
...
class Workspace:
    def create_workspace(self):
        if not os.path.exists(self.workspace_dir):
            print "mkdir %s" % self.workspace_dir
            os.mkdir(self.workspace_dir)
        if not os.path.exists(self.mbed_os_dir):
            old_dir = os.getcwd()
            print "chdir %s" % self.workspace_dir
            os.chdir(self.workspace_dir)
            os_cmd("mbed import mbed-os")
            os.chdir(self.mbed_os_dir)
            print "Correcting requirements.txt..."
            ...
            f.close()
            os.chdir(old_dir)
        if subprocess.check_output(["mbed", "config", "-G", "MBED_OS_DIR"]) == "[mbed] No global MBED_OS_DIR set\n":
            os_cmd("mbed config -G MBED_OS_DIR %s" % self.mbed_os_dir)     

    def __init__(self, workspace_dir):
        self.workspace_dir = os.path.abspath(workspace_dir)
        self.mbed_os_dir = os.path.join(self.workspace_dir, "mbed-os")
